...
sayilar.forEach((num) => {
  sayiTekrarObjesi[num] = (sayiTekrarObjesi[num] || 0) + 1;
});

for (let key in sayiTekrarObjesi) {
  if (sayiTekrarObjesi[key] > 1) {
    tekraredensayilar.push(
      `${key} sayısı ${sayiTekrarObjesi[key]} kere tekrar edilmiştir`
    );
  }
}
